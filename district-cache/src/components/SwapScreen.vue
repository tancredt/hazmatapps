    ...
    <div v-if="showReasonDialog" class="modal-overlay" @click.self="cancelReason">
      <div class="modal-content">
        ...
        <div class="fault-options">
          ...
          >
            <!-- Using radio inputs to enforce single selection natively -->
            <input 
              type="radio" 
              :value="ft.value.trim()" 
...
// 🎯 Logic to split detectors into "Slotted" vs "Overflow"
...
const submitReason = async () => {
  ...
  try {
    // 1. Determine actions and message based on fault type
    ...
    const payload = {
      ...
      fault_data: {
        ...
        report_dt: new Date().toISOString(), // Current datetime
    ...
    // 4. Show info dialog and reset state
...
const closeInfoDialog = () => {
  ...
  // Refresh data to reflect the new locations and statuses
  swap.fetchDetectors()
}
...
.slot-rectangle {
  ...
  /* 🎯 FIX: Prevent mobile tap highlight and text selection color changes */
  -webkit-tap-highlight-color: transparent; 
  user-select: none;
}
.slot-rectangle:hover:not(.empty) { border-color: #42b883; background: #f0fdf4; transform: translateY(-2px); }
/* 🎯 FIX: Changed background to light green, kept text dark (#333) */
